...
export class ListRevenueComponent implements OnInit, OnDestroy {
	...
	isAllSelected(): boolean {
		...
		const numRows = this.revenueResult.length;
		return numSelected === numRows;
	}

	masterToggle() {
		if (this.selection.selected.length === this.revenueResult.length) {
			this.selection.clear();
		} else {
			this.revenueResult.forEach(row => this.selection.select(row));
		}
	}
	...
	onSubmit(){
		...
		this.http.post<any>('http://localhost:3000/api/excel/revenue/import', formData).subscribe(
			res =>{
