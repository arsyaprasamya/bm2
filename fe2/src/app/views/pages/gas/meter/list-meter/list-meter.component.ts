...
export class ListMeterComponent implements OnInit, OnDestroy {
	...
	printGasMeter() {
		...
		this.http.post('http://localhost:3000/api/gas/master/qrcode', id, { responseType: 'arraybuffer' }).subscribe(
			(res) => {
	...
	onSubmit(){
		...
		this.http.post<any>('http://localhost:3000/api/excel/gasmas/import', formData).subscribe(
			res =>{
