...
export class ListFloorComponent implements OnInit, OnDestroy {
	...
	isAllSelected(): boolean {
		...
		const numRows = this.floorResult.length;
		return numSelected === numRows;
	}

	masterToggle() {
		if (this.selection.selected.length === this.floorResult.length) {
			this.selection.clear();
		} else {
			this.floorResult.forEach(row => this.selection.select(row));
		}
	}
	...
	onSubmit(){
		...
		this.http.post<any>('http://localhost:3000/api/excel/floor/import', formData).subscribe(
			res =>{
