...
export class ListUnittypeComponent implements OnInit, OnDestroy {
	...
	displayedColumns = [ 'prnt','billingNo','rentalUnit','rentalTenant','billingDate', 'dueDate','actions'];
	...
	printBilling(id) {
		const API_BILLING_URL = "http://localhost:3000/api/rentalbilling";
	...
	onSubmit(){
		...
		this.http.post<any>('http://localhost:3000/api/excel/rentalbilling/import', formData).subscribe(
			res =>{
