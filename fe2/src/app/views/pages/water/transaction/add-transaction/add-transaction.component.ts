...
export class AddTransactionComponent implements OnInit, OnDestroy {
	...
	addWaterTransaction(
		...
		const addSubscription = this.service
			.createWaterTransaction(_waterTransaction)
			.subscribe(
				(res) => {
				...
				(err) => {
					console.error(err);
					const message =
						"Error while adding water consumption | " +
						err.statusText;
					this.layoutUtilsService.showActionNotification(
						message,
						MessageType.Create,
	...
	// changeMeterPost() {
	// 	const strtpos = this.waterTransactionForm.controls.strtpos.value;
	// 	const endpos = this.waterTransactionForm.controls.endpos.value;
	...
	changeAir() {
		...
		if (billamn !== 0 && air_kotor !== 0 ) {
			this.waterTransactionForm.controls.dwres.setValue(
				Math.fround(billamn * air_kotor).toFixed(2)
			);
		}
	}
	ngOnDestroy() {
