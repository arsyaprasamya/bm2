...
} from "../../../../core/accountBank/accountBank.selector";
...
export class AddAccountBankComponent implements OnInit, OnDestroy {
	...
	reset() {
		this.accountBank = Object.assign({}, this.oldAccountBank);
		this.createForm();
		this.hasFormErrors = false;
		this.accountBankForm.markAsPristine();
		this.accountBankForm.markAsUntouched();
		this.accountBankForm.updateValueAndValidity();
	}
	...
	prepareAccountBank(): AccountBankModel {
		...
		_accountBank.acctName = controls.acctName.value;
	...
	addAccountBank( _accountBank: AccountBankModel, withBack: boolean = false) {
		const addSubscription = this.service.createAccountBank(_accountBank).subscribe(
			res => {
				const message = `New Account successfully has been added.`;
	...
	getComponentTitle() {
		let result = 'Create Account Bank';
		if (!this.accountBank || !this.accountBank._id) {
			return result;
		}

		result = `Edit Account Bank`;
		return result;
	}
