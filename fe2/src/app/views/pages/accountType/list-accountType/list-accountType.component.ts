...
export class ListAccountTypeComponent implements OnInit, OnDestroy {
	...
	displayedColumns = [ 'acctypeno', 'acctype','actions'];
	...
	filterConfiguration(): any {
		...
		const searchNumber: number = this.searchInput.nativeElement.value;

		filter.acctypeno = `${searchNumber}`;
		filter.acctype = `${searchText}`.toLowerCase();
		return filter;
	}
	...
	onSubmit(){
		...
		this.http.post<any>('http://localhost:3000/api/excel/accountType/import', formData).subscribe(
			res =>{
