...
export class ListCustomerComponent implements OnInit, OnDestroy {
	...
	displayedColumns = ['cstrmrcd', 'cstrmrnm', 'gender', 'phone', 'email', 'actions'];
	...
	ngOnInit() {
		...
		**/
		...
		this.subheaderService.setTitle('Tenant');
	...
	deleteCustomer(_item: CustomerModel) {
		...
		const _title = 'Tenant Delete';
		// tslint:disable-next-line:variable-name
		const _description = 'Are you sure to permanently delete this tenant?';
		const _waitDesciption = 'Tenant is deleting...';
		const _deleteMessage = `Tenant has been deleted`;
	...
	onSubmit(){
		...
		this.http.post<any>('http://localhost:3000/api/excel/customer/import', formData).subscribe(
			res =>{
