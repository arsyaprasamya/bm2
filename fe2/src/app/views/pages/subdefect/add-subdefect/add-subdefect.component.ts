...
export class AddSubdefectComponent implements OnInit, OnDestroy {
	...
	createForm() {
		if (this.subdefect._id){
			...
			category: [this.subdefect.defect.category._id, Validators.required],
	...
	reset() {
		this.subdefect = Object.assign({}, this.oldSubdefect);
		this.createForm();
		this.hasFormErrors = false;
		this.subdefectForm.markAsPristine();
		this.subdefectForm.markAsUntouched();
		this.subdefectForm.updateValueAndValidity();
	}
	...
	addSubdefect( _subdefect: SubdefectModel, withBack: boolean = false) {
		const addSubscription = this.service.createSubdefect(_subdefect).subscribe(
			res => {
				const message = `New Subdefect successfully has been added.`;
			...
			err => {
				console.error(err);
				const message = 'Error while adding subdefect | ' + err.statusText;
	...
	getComponentTitle() {
		let result = 'Create Sub Detail Location';
		if (!this.subdefect || !this.subdefect._id) {
			return result;
		}

		result = `Edit Sub Detail Location`;
		return result;
	}
