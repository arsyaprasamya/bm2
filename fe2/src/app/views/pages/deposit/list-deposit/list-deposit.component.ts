...
import { ViewComponent } from '../view-deposit/view-edit.component';
...
export class ListDepositComponent implements OnInit, OnDestroy {
	...
	printDeposit(id) {
		...
		const API_DEPOSIT_URL = "http://localhost:3000/api/deposit";
	...
	onSubmit(){
		...
		this.http.post<any>('http://localhost:3000/api/excel/deposit/import', formData).subscribe(
			(res) => console.log(res),
			(err) => console.log(err)
		)
	}
	...
	// viewComponent() {
	// 	const newCustomer = new DepositModel();
	...
	//  */
	editCustomer(id: DepositModel) {
		let saveMessageTranslateParam = 'ECOMMERCE.CUSTOMERS.EDIT.';
		saveMessageTranslateParam += id != null ? 'UPDATE_MESSAGE' : 'ADD_MESSAGE';
		const _saveMessage = this.translate.instant(saveMessageTranslateParam);
		const _messageType = id != null ? MessageType.Update : MessageType.Create;
		const dialogRef = this.dialog.open(ViewComponent, { data: { id } });
		dialogRef.afterClosed().subscribe(res => {
			if (!res) {
				return;
			}
