...
export class AddDepositComponent implements OnInit, OnDestroy {
	...
	ngOnInit() {
		...
		const routeSubscription =  this.activatedRoute.params.subscribe(params => {
			...
			this.deposit.clear();
			this.initDeposit();
			// this.datedefault = new Date().toLocaleDateString('en-GB');
			// this.date = this.datedefault.replace(/T.*/,'');
		});
	...
	getInvoice(id){
		const controls = this.depositForm.controls;
		 this.invService.findInvoiceById(id).subscribe(data =>{
			...
			controls.total.setValue(data.data.amount -  (data.data.amount / 10))
		 	}
		 );
	}
