...
export class ListUnitrateComponent implements OnInit, OnDestroy {
	...
	ngOnInit() {
		...
		**/
		...
		const searchSubscription = fromEvent(this.searchInput.nativeElement, 'keyup').pipe(
			...
			debounceTime(100), // The user can type quite quickly in the input box, and that could trigger a lot of server requests. With this operator, we are limiting the amount of server requests emitted to a maximum of one every 150ms
	...
	fetchUnitRate() {
		...
		this.selection.selected.forEach(elem => {
			messages.push({
				text: `${elem.service_rate} , Unit Rate Service: ${elem.sinking_fund} , Unit Rate Sinking Fund: ${elem.overstay_rate}`,
				id: elem._id.toString(),
				status: elem.service_rate
			});
		});
	...
	isAllSelected(): boolean {
		...
		const numRows = this.unitRateResult.length;
		return numSelected === numRows;
	}

	masterToggle() {
		if (this.selection.selected.length === this.unitRateResult.length) {
			this.selection.clear();
		} else {
			this.unitRateResult.forEach(row => this.selection.select(row));
		}
	}
	...
	onSubmit(){
		...
		this.http.post<any>('http://localhost:3000/api/excel/unitrate/import', formData).subscribe(
			res =>{
