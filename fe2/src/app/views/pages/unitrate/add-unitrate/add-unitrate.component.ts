...
export class AddUnitRateComponent implements OnInit, OnDestroy {
	...
	if (this.unitRate._id){
			...
			overstay_rate: [this.unitRate.overstay_rate, Validators.required],
		});
		}else{
			this.unitRateForm = this.unitRateFB.group({
			...
			isRent : [this.unitRate.isRent],
			rentPrice : [""],
			overstay_rate: ["", Validators.required],
			});
		}
	}
	...
	reset() {
		this.unitRate = Object.assign({}, this.oldUnitRate);
		this.createForm();
		this.hasFormErrors = false;
		this.unitRateForm.markAsPristine();
		this.unitRateForm.markAsUntouched();
		this.unitRateForm.updateValueAndValidity();
	}
