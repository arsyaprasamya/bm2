...
export class ListRateComponent implements OnInit, OnDestroy {
	...
	fetchPowerRate() {
		...
		this.selection.selected.forEach(elem => {
			messages.push({
				text: `${elem.nmrtepow} , Rate: Rp. ${elem.rte}`,
				id: elem._id.toString(),
				status: elem.nmrtepow
			});
		});
	...
	isAllSelected(): boolean {
		...
		const numRows = this.powerRateResult.length;
		return numSelected === numRows;
	}

	masterToggle() {
		if (this.selection.selected.length === this.powerRateResult.length) {
			this.selection.clear();
		} else {
			this.powerRateResult.forEach(row => this.selection.select(row));
		}
	}
