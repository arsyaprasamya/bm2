...
export class ListMeterComponent implements OnInit, OnDestroy {
	...
	printPowerMeter() {
		...
		this.http.post('http://localhost:3000/api/power/master/qrcode', id, { responseType: 'arraybuffer' }).subscribe(
			(res) => {
	...
	onSubmit(){
		...
		this.http.post<any>('http://localhost:3000/api/excel/powermas/import', formData).subscribe(
			res =>{
