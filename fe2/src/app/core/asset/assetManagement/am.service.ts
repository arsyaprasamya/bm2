...
const API_FLOOR_URL = 'http://localhost:3000/api/asset/management';
const API_CSV = 'http://localhost:3000/api/excel/asset/management/export';
...
export class AmService {
	...
	deleteAm(amId: string) {
		const url = `${API_FLOOR_URL}/${amId}`;
		return this.http.delete(url);
	}
	updateAm(am: AmModel) {
		const url = `${API_FLOOR_URL}/${am._id}`;
		return this.http.patch(url, am);
	}
	createAm(am: AmModel): Observable<AmModel>{
		const httpHeaders = new HttpHeaders();
		httpHeaders.set('Content-Type', 'application/json');
		return this.http.post<AmModel>(`${API_FLOOR_URL}`, am, { headers: httpHeaders});
	}
