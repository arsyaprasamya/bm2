...
const API_LB_URL = 'http://localhost:3000/api/leasebilling';
const API_CSV = 'http://localhost:3000/api/excel/leasebilling/export';
...
export class LsebillingService {
	...
	deleteLsebilling(lsebillingId: string) {
		const url = `${API_LB_URL}/${lsebillingId}`;
		return this.http.delete(url);
	}
	updateLsebilling(lsebilling: LsebillingModel) {
		const url = `${API_LB_URL}/${lsebilling._id}`;
