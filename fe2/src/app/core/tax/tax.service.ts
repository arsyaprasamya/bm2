...
const API_FLOOR_URL = 'http://localhost:3000/api/tax';
const API_CSV = 'http://localhost:3000/api/excel/tax/export';
...
export class TaxService {
	...
	deleteTax(taxId: string) {
		const url = `${API_FLOOR_URL}/${taxId}`;
		return this.http.delete(url);
	}
	updateTax(tax: TaxModel) {
		const url = `${API_FLOOR_URL}/${tax._id}`;
		return this.http.patch(url, tax);
	}
	createTax(tax: TaxModel): Observable<TaxModel>{
		const httpHeaders = new HttpHeaders();
		httpHeaders.set('Content-Type', 'application/json');
		return this.http.post<TaxModel>(`${API_FLOOR_URL}`, tax, { headers: httpHeaders});
	}
