...
const API_FLOOR_URL = 'http://localhost:3000/api/parkingbilling';
const API_CSV = 'http://localhost:3000/api/excel/parkingbilling/export';
...
export class PrkbillingService {
	...
	generateCodeLeaseBiling(): Observable<QueryResultsModel>{
		const url = `${API_FLOOR_URL}/generate/CodeLeaseBiling`;
		return this.http.get<QueryResultsModel>(url);
	}

	
	findPrkbillingById(_id: string): Observable<PrkbillingModel>{
		return this.http.get<PrkbillingModel>(`${API_FLOOR_URL}/${_id}`);
	}
	deletePrkbilling(prkbillingId: string) {
		const url = `${API_FLOOR_URL}/delete/${prkbillingId}`;
		return this.http.delete(url);
	}
	updatePrkbilling(prkbilling: PrkbillingModel) {
		const url = `${API_FLOOR_URL}/${prkbilling._id}`;
		return this.http.patch(url, prkbilling);
	}
	createPrkbilling(prkbilling: PrkbillingModel): Observable<PrkbillingModel>{
		const httpHeaders = new HttpHeaders();
		httpHeaders.set('Content-Type', 'application/json');
		return this.http.post<PrkbillingModel>(`${API_FLOOR_URL}/add`, prkbilling, { headers: httpHeaders});
	}
