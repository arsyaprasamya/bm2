const {listDefect, lstDefectById, addDefect, editDefect, deleteDefect, GenerateDefectCode, 
    findDefectByParent, listCategory, lstCategoryById} = require("./service");
...
module.exports = {
    Defect: async function (req, res, next) {
        try {
           ...
           if (defect) {
               return res.status(200).json({"status": "success", "data": defect, "totalCount": allData.length});
            } else {
    ...
    listCategory: async function (req, res, next) {
        try {
            const list = await listCategory(req.params.filter, req.params.page, req.params.limit);
            if (list) {
