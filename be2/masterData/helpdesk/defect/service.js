...
module.exports = {
    listDefect: async function(query, page= 1, limit = 10){
        try {
            ...
            if(Object.keys(query).length === 0){
                defect = await Defect.find().skip(skip).limit(limit).populate("category", "-__v").select('-__v').sort({$natural:-1});
            }else{
                defect = await Defect.find().or([{
                    'defect_name': { $regex: query.defect_name }
                }]).skip(skip).limit(limit).populate("category", "-__v").select('-__v').sort({$natural:-1});
            }
            if(defect){
                return defect;
            }else{
    ...
    findDefectByParent: async function(id){
        try {
            const defect = await Defect.find({category: id}).select("-__v");
            if(defect){
                return defect;
            }else{
                return false;
    ...
    listCategory : async function(query, page = 1, limit = 1000){
        try {
            const skip = (page - 1) * limit;
            const category = await Category.find(query).skip(skip).limit(limit).populate({path:'Category'});
            if(category){
                return category;
            }else{
                return false;
