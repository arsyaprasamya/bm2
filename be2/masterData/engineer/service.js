...
module.exports = {
    listEngineer: async function(query, page= 1, limit = 1000){
        try {
            ...
            // let engineer = await Engineer.find(query).skip(skip).limit(limit).sort({$natural:-1});
            let engineer;
            if (Object.keys(query).length === 0){
                engineer = await Engineer.find().skip(skip).limit(limit).select('-__v').sort({$natural:-1});
            }else{
                engineer = await Engineer.find().or([
