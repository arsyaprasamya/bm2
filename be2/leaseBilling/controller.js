...
const { addBilling, listLeaseBilling, getBillingById, getBillingNumber, deleteBilling } = require("./service");
const { LeaseBilling } = require('./model');
...
module.exports = {
    generateCodeLeaseBiling : async function (req, res, next) {
        try {
          ...
          const code = "LS-" + date + month + year + "-" + newNumber;
    ...
    addBill : async function (req, res, next) {
        try {
          const dates = new Date();
          const createBilling = await LeaseBilling.create(req.body);
    ...
    listBill: async function (req, res, next) {
        try {
            ...
            const allData = await listLeaseBilling({}, 1, 0);
            let query = {};
            if(str.filter !== null) query = {billingNo: str.filter.billingNo};
            const bill = await listLeaseBilling(query, str.pageNumber, str.limit);
            if (bill) {
