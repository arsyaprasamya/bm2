...
const { listParkBilling, getBillingById, deleteBilling } = require("./service");
const { ParkBilling } = require('./model');
...
module.exports = {
  generateCodeLeaseBiling : async function (req, res, next) {
      try {
        ...
        const code = "PB -" + date + month + year + "-" + newNumber;
  ...
  addBill : async function (req, res, next) {
      try {
        const dates = new Date();
        const createBilling = await ParkBilling.create(req.body);
  ...
  listBill: async function (req, res, next) {
      try {
          ...
          const allData = await listParkBilling({}, 1, 0);
          let query = {};
          if(str.filter !== null) query = {billingNo: str.filter.billingNo};
          const bill = await listParkBilling(query, str.page, str.limit);
          if (bill) {
