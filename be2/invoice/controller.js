...
const {addInvoice, findAllInvoice, updateInvoice, deleteInvoice, findByInvoiceId, } = require("./service");
...
module.exports = {
    generateCodeInvoice : async function (req, res, next) {
        try {
          ...
          const code = "INV - " + date + month + year + " - " + newNumber;
    ...
    tambahInvoice : async function(req, res, next){
    ...
    listInvoice : async function(req, res, next){
        const str = JSON.parse(req.query.param);
        const allData = await findAllInvoice({}, 1, 0);
        let query = {};
        if(str.filter !== null) query = {invoiceno: str.filter.invoiceno};
        const listinvoice = await findAllInvoice(query, str.pageNumber, str.limit);
        if(listinvoice){
            return res.status(200).json({"status": "success", "data": listinvoice, "totalCount": allData.length});
        }else{
            return res.status(500).json({"status": "error", "data": "internal server error"});
        }
    },

    editInvoice : async function(req, res, next){
        const upd = await updateInvoice(req.params.id, req.body);
        if(upd){
    ...
    HapusInvoice : async function(req, res, next){
        const del = await deleteInvoice(req.params.id);
        if(del){
    ...
    getInvoiceByID : async function(req, res, next){
        const invoice = await findByInvoiceId(req.params.id);
        if(invoice){
            return res.status(200).json({"status": "success", "data": invoice});
        }else{
            return res.status(500).json({"status": "error", "data": "internal server error"});
    ...
    createInvoice: async function (req, res, next) {
        try {
            const inv = await findByInvoiceId(req.params.id);
            const name = await Date.now();
            ejs.renderFile(path.join(__dirname, './template', "invoice.template.ejs"), { invoice: inv, moment: moment, terbilang: terbilang }, (err, data) => {
                if (err) {
                    console.log(err);
    ...
    createInvoiceAdmin: async function (req, res, next) {
        try {
            const inv = await findByInvoiceId(req.params.id);
            const name = await Date.now();
            ejs.renderFile(path.join(__dirname, './template', "invoiceadmin.template.ejs"), { invoice: inv, moment: moment, terbilang: terbilang }, (err, data) => {
                if (err) {
                    console.log(err);
                    return res.status(500).json({ status: "error", data: err });
                } else {
